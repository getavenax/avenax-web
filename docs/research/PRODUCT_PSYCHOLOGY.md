...
| **Known Limitations** | Framework-level synthesis; AVENAX-specific behavioral data does not exist until Trace ships with its own (consented) telemetry |
...
**AVENAX fit:** Trace's north-star behavior is already defined this way (PRD retention metric: ≥1 chart/week in month 2 — context-tied). **Action:** when Phase 1 telemetry is designed (NFR-8), instrument "analyses per trading session" not "session count."
...
**AVENAX fit:** the Reasoning section is the competence engine — Trace must make users *better readers of charts*, visibly, or it becomes a dependency product with dependency churn. **Ethics:** teaching-first is the white-hat retention strategy by definition. **Action:** already MVP scope; protect it in every scope negotiation.
